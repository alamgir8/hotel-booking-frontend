...
export const checkRoomBooking =
  (id: IRoom['_id'], checkInDate: Date, checkOutDate: Date) =>
  async (dispatch: Dispatch) => {
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/api/bookings/check`,
...
export const getBookedDates =
  (roomId: IRoom['_id']) => async (dispatch: Dispatch) => {
    try {
      dispatch({ type: actions.GET_BOOKED_DATES_REQUEST });

      const { data } = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/bookings/dates/${roomId}`
      );
...
export const getMyBookings =
  () => async (dispatch: Dispatch, getState: any) => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/bookings/me`,
        config
      );
...
export const getAllBookings =
  (currentPage: number) => async (dispatch: Dispatch, getState: any) => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/bookings/?pageNumber=${currentPage}`,
        config
      );
...
export const deleteBooking =
  (bookingId: IBooking['_id']) => async (dispatch: Dispatch, getState: any) => {
    try {
