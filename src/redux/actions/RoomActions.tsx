  ...
  async (dispatch: Dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/rooms/?keyword=${keyword}&numOfBeds=${numOfBeds}&roomType=${roomType}&pageNumber=${currentPage}`
      );
...
export const getRoomDetails =
  (id: IRoom['_id']) => async (dispatch: Dispatch) => {
    try {
      dispatch({ type: actions.ROOM_DETAILS_REQUEST });

      const { data } = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/api/rooms/${id}`
      );
...
export const createRoomReview =
  (id: IRoom['_id'], review: ICreateReview) =>
  async (dispatch: Dispatch, getState: any) => {
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/api/rooms/${id}/reviews`,
        review,
        config
      );
...
export const createRoom =
  (roomData: TCreateRoom) => async (dispatch: Dispatch, getState: any) => {
    try {
      ...
      await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/api/rooms`,
        roomData,
        config
      );
...
export const updateRoom =
  (roomId: IRoom['_id'], roomData: TCreateRoom) =>
  async (dispatch: Dispatch, getState: any) => {
    try {
      ...
      await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/api/rooms/${roomId}`,
        roomData,
        config
      );
...
export const deleteRoom =
  (roomId: IRoom['_id']) => async (dispatch: Dispatch, getState: any) => {
    try {
